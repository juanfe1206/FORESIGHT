...
| C2 — AgentHUD center | Epic 5 | `src/lib/integration-contracts.ts` | `AgentHudSlotProps` | `MOCK_AGENT_HUD_PROPS` | Four `"dormant"` agent states + `AGENT_ROLES["map"]` roles. Represents pre-run stage; use for HUD layout development without a running simulation. | `npm run test -- src/lib/integration-contracts.test.ts` |
| C3 — KPI stack center | Epic 5 | `src/lib/integration-contracts.ts` | `KpiStackSlotProps` | `MOCK_KPI_STACK_PROPS` | Both path KPIs, comparison, and path labels from the bakery scenario. Use for dashboard/KPI stack development without a live result. | `npm run test -- src/lib/integration-contracts.test.ts` |
...
- UI slot contracts (C1, C3–C5) use typed props plus mocks derived from `MOCK_BAKERY_MAP_FIXTURE`. `MOCK_AGENT_HUD_PROPS` (C2) uses hand-built dormant states representing the pre-run simulation stage.
