...
|-------------|------------|-------------|----------------------|---------------|------------------------|--------------|
| C1 — Viz slot | Epic 3 (FlowView), Epic 4 (MapView), Epic 5 (NetworkView, FallbackViz) | `src/lib/integration-contracts.ts` | `VizSlotProps`, `FallbackVizProps` | `MOCK_VIZ_SLOT_PROPS_A`, `MOCK_VIZ_SLOT_PROPS_NETWORK` (`viz_type: "network"`), `MOCK_FALLBACK_VIZ_PROPS_A` (`FallbackViz` + dormant agents) | Thin-slice router: `src/components/viz/VizRouter.tsx` → `src/components/viz/NetworkView/NetworkView.tsx`, `src/components/viz/FallbackViz.tsx`. Map/flow placeholders until Epic 3/4. | `npm run test -- src/lib/integration-contracts.test.ts` |
...
| C4 — ScoreRing side | Epic 5 | `src/lib/integration-contracts.ts` | `ScoreRingSlotProps` | `MOCK_SCORE_RING_PROPS_A`, `MOCK_SCORE_RING_PROPS_B` | Path A/B overall scores (67 / 78) and labels; UI: `ScoreRing` in `src/components/dashboard/ScoreRing.tsx`. Scores are clamped to **0–100** at render if the API sends out-of-range values. | `npm run test -- src/lib/integration-contracts.test.ts` |
